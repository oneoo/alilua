...
int lua_f_fastlz_compress(lua_State *L){
	...
	if(vlen < 1 || vlen > 10*1024*1024){ /// Max 10Mb !!!
		lua_pushnil(L);
		return 1;
	}
...
int lua_f_fastlz_decompress(lua_State *L){
	...
	char *dst = &temp_buf;
	if(value_len+2 > 4096)
		dst = (unsigned char *)large_malloc(value_len+2);
	...
	int dlen = fastlz_decompress(value+sizeof(unsigned int), vlen, dst, value_len+2);
